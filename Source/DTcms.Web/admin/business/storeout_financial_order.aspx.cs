...
{
    public partial class storeout_financial_order : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("storeout_financial_order", DTEnums.ActionEnum.View.ToString()); //检查权限
                TreeBind("");
        ...
        private void RptBind(string _strWhere, string _goodsby)
        {
            ...
            string pageUrl = Utils.CombUrlTxt("storeout_financial_order.aspx", "customer_id={0}&keyword={1}&beginTime={2}&endTime={3}&page={4}",
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("storeout_financial_order.aspx", "customer_id={0}&keyword={1}&beginTime={2}&endTime={3}",
        ...
        {
            ChkAdminLevel("storeout_financial_order", DTEnums.ActionEnum.Confirm.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.StoreOutOrder bll = new BLL.StoreOutOrder();
            ...
            JscriptMsg("确认出库单成功" + sucCount + "条，失败" + errorCount + "条！", Utils.CombUrlTxt("storeout_financial_order.aspx", "customer_id={0}&keyword={1}&beginTime={2}&endTime={3}",
                this.customer_id.ToString(), this.keyword.ToString(), this.beginTime.ToString(), this.endTime));
        }
