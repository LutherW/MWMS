...
{
    public partial class received_money_list : Web.UI.ManagePage
    {
        ...
        #region 组合SQL查询语句==========================
        protected string CombSqlTxt(int _customer_id, int _storein_order_id, int _vehicle_id, string _beginTime, string _endTime, string _keywords)
        {
            ...
            {
                strTemp.Append(" and A.Name like '%" + _keywords + "%'");
            }
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("received_money_page_size", "DTcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("received_money_list.aspx", "customer_id={0}&storein_order_id={1}&vehicle_id={2}&beginTime={3}&endTime={4}&keywords={5}",
                this.customer_id.ToString(), this.storein_order_id.ToString(), this.vehicle_id.ToString(), txtBeginTime.Text, txtEndTime.Text, txtKeyWord.Text));
        }


        protected void ddlCustomer_SelectedIndexChanged(object sender, EventArgs e)
        {
            StoreInOrderBind(ddlStoreInOrder.SelectedValue);
            Response.Redirect(Utils.CombUrlTxt("received_money_list.aspx", "customer_id={0}&storein_order_id={1}&vehicle_id={2}&beginTime={3}&endTime={4}&keywords={5}",
                ddlCustomer.SelectedValue, this.storein_order_id.ToString(), this.vehicle_id.ToString(), this.beginTime.ToString(), this.endTime, this.keywords.ToString()));
        }
        ...
        protected void ddlStoreInOrder_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            Response.Redirect(Utils.CombUrlTxt("received_money_list.aspx", "customer_id={0}&storein_order_id={1}&vehicle_id={2}&beginTime={3}&endTime={4}&keywords={5}",
                this.customer_id.ToString(), ddlStoreInOrder.SelectedValue, this.vehicle_id.ToString(), this.beginTime.ToString(), this.endTime, this.keywords.ToString()));
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("received_money_page_size", "DTcmsPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("received_money_list.aspx", "customer_id={0}&storein_order_id={1}&vehicle_id={2}&beginTime={3}&endTime={4}&keywords={5}",
                this.customer_id.ToString(), this.storein_order_id.ToString(), this.vehicle_id.ToString(), this.beginTime.ToString(), this.endTime, this.keywords.ToString()));
        }
