...
{
    public partial class storeout_goods_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("storeout_goods", DTEnums.ActionEnum.View.ToString()); //检查权限
                TreeBind("");
                RptBind(CombSqlTxt(this.customer_id, this.storein_order_id, this.keyword, this.beginTime, this.endTime), " B.StoredOutTime DESC ");
            }
        }
        ...
        private void RptBind(string _strWhere, string _goodsby)
        {
            ...
            BLL.StoreOutGoods bll = new BLL.StoreOutGoods();
            this.rptList.DataSource = bll.GetSearchList(this.pageSize, this.page, _strWhere, _goodsby, out this.totalCount);
            this.rptList.DataBind();

            //绑定页码
            txtPageNum.Text = this.pageSize.ToString();
            string pageUrl = Utils.CombUrlTxt("storeout_goods_list.aspx", "customer_id={0}&storein_order_id={1}&keyword={2}&beginTime={3}&endTime={4}&page={5}",
        ...
        protected string CombSqlTxt(int _customer_id, int _storein_order_id, string _keyword, string _beginTime, string _endTime)
        {
            ...
            {
                strTemp.Append(" and (GoodsName like '%" + _keyword + "%' or StoreName like '%" + _keyword + "%' or B.Admin = '" + _keyword + "')");
            }
            if (!string.IsNullOrEmpty(beginTime))
            {
                strTemp.Append(" and B.StoredOutTime>='" + _beginTime + "'");
            }
            if (!string.IsNullOrEmpty(endTime))
            {
                strTemp.Append(" and B.StoredOutTime <='" + _endTime + "'");
            }
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("storeout_goods_page_size", "DTcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("storeout_goods_list.aspx", "customer_id={0}&storein_order_id={1}&keyword={2}&beginTime={3}&endTime={4}",
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("storeout_goods_page_size", "DTcmsPage", _pagesize.ToString(), 14400);
                }
            }
