...
{
    public partial class allot_goods_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("allot_goods", DTEnums.ActionEnum.View.ToString()); //检查权限
                TreeBind("");
                RptBind(CombSqlTxt(this.customer_id, this.storein_order_id, this.keyword, this.beginTime, this.endTime), " B.AllotTime DESC ");
            }
        }
        ...
        protected string CombSqlTxt(int _customer_id, int _storein_order_id, string _keyword, string _beginTime, string _endTime)
        {
            ...
            if (_customer_id > 0)
            {
                strTemp.Append(" and D.CustomerId=" + _customer_id);
            }
            ...
            {
                strTemp.Append(" and B.Admin like '%" + _keyword + "%'");
            }
            if (!string.IsNullOrEmpty(beginTime))
            {
                strTemp.Append(" and B.AllotTime>='" + _beginTime + "'");
            }
            if (!string.IsNullOrEmpty(endTime))
            {
                strTemp.Append(" and B.AllotTime <='" + _endTime + "'");
            }
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("allot_goods_page_size", "DTcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("allot_goods_list.aspx", "customer_id={0}&storein_order_id={1}&keyword={2}&beginTime={3}&endTime={4}",
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("allot_goods_page_size", "DTcmsPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("allot_goods_list.aspx", "customer_id={0}&storein_order_id={1}&keyword={2}&beginTime={3}&endTime={4}",
