...
namespace DTcms.BLL
{
    ...
    public partial class Vehicle
    {
        ...
        /// 得到一个对象实体，从缓存中
        /// </summary>
        //public DTcms.Model.Vehicle GetModelByCache(int Id)
        //{

        //    string CacheKey = "VehicleModel-" + Id;
        ...
        //        catch{}
        //    }
        //    return (DTcms.Model.Vehicle)objModel;
        //}
        ...
        /// 获得数据列表
        /// </summary>
        public List<DTcms.Model.Vehicle> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<DTcms.Model.Vehicle> DataTableToList(DataTable dt)
        {
            List<DTcms.Model.Vehicle> modelList = new List<DTcms.Model.Vehicle>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                DTcms.Model.Vehicle model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new DTcms.Model.Vehicle();
                    if (dt.Rows[n]["Id"].ToString() != "")
                    {
                        model.Id = int.Parse(dt.Rows[n]["Id"].ToString());
                    }
                    model.PlateNumber = dt.Rows[n]["PlateNumber"].ToString();
